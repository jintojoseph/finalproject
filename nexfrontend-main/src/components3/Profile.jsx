...
const Profile = () => {
  ...
  const handleClose = () => {
    ...
    useEffect(()=>{
      axios.get('http://localhost:4000/mindmaps').then((res)=>{
        setRows(res.data);
      })
    },[])

    function deleteData(p){
      axios.delete('http://localhost:4000/mindmapsremove/'+p).then((res)=>{
  ...
  let updateUser=(mindmaps)=>{
    navigate('/reg',{state:{mindmaps}})
  }
        ...
        <Dialog open={open} onClose={handleClose} className="custom-dialog">
          ...
          <DialogContent>
            <div className="image-gallery">
              <img src="profile100.png" alt="Image 1" onClick={() => {
        setSelectedImage('profile100.png');
        setOpen(false);
      }} />
              <img src="profile2.png" alt="Image 2" onClick={() => {
        setSelectedImage('profile2.png');
        setOpen(false);
      }} />
        ...
        <TableBody>
          {rows.map((row) => (
            ...
            >
              ...
              <TableCell align="right"><Button variant='contained' color='inherit'onClick={()=>{updateUser(row)}} sx={{ backgroundColor: '#6b1ca2', '&:hover': { backgroundColor: '#6b2ca2' } }} >Edit</Button></TableCell>
              <TableCell align="right"><Button variant='contained' color='inherit'onClick={()=>{deleteData(row._id)}}sx={{ backgroundColor: '#6b1ca2', '&:hover': { backgroundColor: '#6b2ca2' } }}>Delete</Button></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
